<#
.SYNOPSIS 
    Deploys the Hyper-V Sandbox - vMode Edition: a nested Hyper-V Windows Server lab for
    learning and validating Active Directory, Failover Clustering, SMB, Storage Spaces Direct,
    Windows Admin Center (including Virtualization Mode / "vMode"), and Software-Defined
    Networking (SDN). For training and validation only - not supported for production use.

.EXAMPLE
    .\New-HyperVSandbox.ps1
    Reads in the configuration from SDNSandbox-Config.psd1 that contains a hash table 
    of settings data that will in same root as New-HyperVSandbox.ps1
  
.EXAMPLE
    .\New-HyperVSandbox.ps1 -Delete $true
     Removes the VMs and VHDs of the Hyper-V Sandbox installation. (Note: Some files will
...
function Get-ScriptDriveRoot {
    # The drive this script lives on (e.g. 'E:'). Used to auto-locate the parent VHDXs when the
    # configured path points at a drive that doesn't exist here. Mirrors the launch-drive
    # auto-detection that New-SDNVHDfromISO.ps1 uses when it builds the images.
    $root = $null
    if ($PSScriptRoot) { try { $root = Split-Path $PSScriptRoot -Qualifier } catch {} }
    if ([string]::IsNullOrWhiteSpace($root)) { try { $root = (Get-Location).Drive.Name + ':' } catch {} }
    if ([string]::IsNullOrWhiteSpace($root)) { $root = $env:SystemDrive }
    return $root
}

function ConvertTo-ScriptDriveRootedPath {
    # Re-base a rooted local path onto a different drive, preserving folders and filename.
    # Relative or UNC paths are returned unchanged. Pure string work, so the target drive need
    # not exist on the machine evaluating it.
...
function Resolve-HostVMPath {
    <#
        Returns the folder where the nested VM files are created. Unlike the parent VHDX paths, this
    ...
    # Honor the configured path when its drive exists (override wins).
...
function Disable-OfflinePrivacyExperience {
    <#
        ...
        offline (mounted) image so first boot does not block waiting for interactive input. Server
        2025 ignores the legacy SkipMachineOOBE/SkipUserOOBE unattend flags for this screen, so the
        documented Group Policy values are written directly into the offline SOFTWARE hive. Applies to
        both Server Core (the numbered diagnostic-data prompt) and Desktop/GUI (the privacy settings
        page). Uses reg.exe (out-of-process) so no lingering .NET hive handles block the unload.
    #>
    ...
    if (-not (Test-Path -LiteralPath $hivePath)) {
        Write-Verbose "Offline SOFTWARE hive not found at $hivePath; skipping privacy-experience suppression."
        return
    }

    $mountKey = 'SDN_OFFLINE_SW'
    $loadOut = reg load "HKLM\$mountKey" "$hivePath" 2>&1
    if ($LASTEXITCODE -ne 0) {
        Write-Verbose "Could not load offline hive ($hivePath): $loadOut. Skipping privacy-experience suppression."
        return
    }

    try {
        # "Don't launch privacy settings experience on user logon" - suppresses the diagnostic-data page.
        $null = reg add "HKLM\$mountKey\Policies\Microsoft\Windows\OOBE" /v DisablePrivacyExperience /t REG_DWORD /d 1 /f 2>&1
        # Pre-set the diagnostic data level to Required so nothing prompts for it.
        $null = reg add "HKLM\$mountKey\Policies\Microsoft\Windows\DataCollection" /v AllowTelemetry /t REG_DWORD /d 1 /f 2>&1
        Write-Verbose "Disabled the OOBE privacy experience in the offline image on $WindowsDrive`:."
    }
    finally {
        # Release the hive so the subsequent Dismount-VHD succeeds; retry because the unload can lag.
        [gc]::Collect()
        $unloaded = $false
        for ($i = 0; $i -lt 5 -and -not $unloaded; $i++) {
            Start-Sleep -Milliseconds 400
            $unloadOut = reg unload "HKLM\$mountKey" 2>&1
            if ($LASTEXITCODE -eq 0) { $unloaded = $true }
        }
        if (-not $unloaded) { Write-Verbose "Could not unload offline hive HKLM\$mountKey after retries: $unloadOut" }
    }
}

function Copy-LargeFile {
    <#
        Copies a large file (e.g. a 20+ GB parent VHDX) using robocopy with unbuffered I/O (/J).
        For multi-GB files this is faster than Copy-Item and, importantly, avoids flooding the
        Windows file cache - which otherwise evicts the working set and slows the rest of the
        deployment running on the same box. Falls back to Copy-Item if robocopy is unavailable or
        reports a failure, so behavior is never worse than the original.
    #>
    param(
    ...
    if (Test-Path $robocopy) {
        ...
        if ($LASTEXITCODE -lt 8) {
            # robocopy preserves the source name. If the caller asked for a genuinely different
            # destination name (case-only differences are already satisfied on NTFS), finalize it.
    ...
    # Fallback: never worse than the original Copy-Item behavior.
...
function Add-Files {
    ...
    foreach ($sdnHOST in $VMPlacement) {
        ...
        $mountResult = Mount-VHD -Path $path -Passthru
        Start-Sleep -Seconds 5

        # Parent images produced by New-SDNVHDfromISO.ps1 are GPT/UEFI, so mounting them exposes
        # MORE THAN ONE lettered volume (the EFI System Partition plus the Windows partition, and
        # sometimes a Recovery partition). Collapsing all of those drive letters into a single
        # string used to yield bogus values like "IK". Select the volume that actually contains
        # \Windows so we always target the OS partition. This also works for a legacy
        # single-partition (MBR) image, which simply has one matching volume.
        $osVolume = $mountResult | Get-Disk | Get-Partition | Get-Volume |
            Where-Object { $_.DriveLetter -and (Test-Path -LiteralPath ("$($_.DriveLetter):\Windows")) } |
            Select-Object -First 1

        if (-not $osVolume) {
            Dismount-VHD -Path $path -ErrorAction SilentlyContinue
            throw "Unable to locate the Windows partition on the mounted VHDX '$path'. None of the mounted volumes contained a \Windows folder."
        }

        [string]$MountedDrive = $osVolume.DriveLetter
...
"@
        ...
        # Suppress the OOBE privacy / "send diagnostic data to Microsoft" screen so first boot does
        # not block on interactive input (the legacy unattend skip flags no longer cover it on 2025).
        Disable-OfflinePrivacyExperience -WindowsDrive $MountedDrive
        ...
        if ($sdnHOST.SDNHOST -eq "SDNMGMT") {
            ...
            Copy-LargeFile -Source $guiVHDXPath -Destination ($MountedDrive + ":\VMs\Base\GUI.vhdx")
...
function New-AdminCenterVM {
    ...
    Invoke-Command -VMName SDNMGMT -Credential $localCred -ScriptBlock {
        ...
        robocopy C:\VMs\Base C:\TempWACMount\VHDs CORE.vhdx GUI.vhdx /J /NP /NFL /NDL /NJH /NJS /R:2 /W:5 | Out-Null
...
# Dot-source guard: when this file is dot-sourced (tests load the functions above via
# ". .\New-HyperVSandbox.ps1"), return before executing the deployment. Running the script
# directly (.\New-HyperVSandbox.ps1) or via the New-SDNSandbox.ps1 compatibility shim keeps
# $MyInvocation.InvocationName as the script name, so deployment proceeds normally.
...
if ($Delete) {
    ...
    Write-Verbose "Successfully Removed the Hyper-V Sandbox"
    exit
...
$NestedVMMemoryinGB = $SDNConfig.NestedVMMemoryinGB
# Resolve the parent images: honor the configured path if it exists, otherwise auto-locate them
# on the drive this wizard is running from (so a hard-coded drive letter in the config that no
...
$lnk.Description = "AdminCenter link for Hyper-V Sandbox - vMode Edition."
$lnk.Save()
